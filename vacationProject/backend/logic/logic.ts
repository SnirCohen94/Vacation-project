...
async function getAllUsers(): Promise<UserModel[]> {
  const sql = `SELECT * FROM vacationsproject.user;`;
  const result = await dal.execute(sql);
  return result;
}
// login
async function login(userLogin: UserModel):Promise<string> {
  const sql = `SELECT * FROM vacationsproject.user WHERE user_name = '${userLogin.user_name}' and user_password='${userLogin.user_password}';`;
...
async function addUser(user: UserModel){
  ...
  const sql = `
  INSERT INTO vacationsproject.user VALUES (DEFAULT,'${user.first_name}', '${user.last_name}', '${user.user_name}', '${user.user_password}',${user.is_admin = Role.User});
  `;
...
async function getAllVacations(): Promise<UserModel[]> {
  const sql = `SELECT id, destination, from_date, to_date, description, CONVERT(image USING utf8) as image, followers, price from vacationsproject.vacation`;
...
async function getOneVacation(id: number): Promise<VacationModel>{
  const sql = `
  SELECT id, destination, from_date, to_date, description, image, followers, price from vacationsproject.vacation WHERE id = ${id}`;
  const vacations = await dal.execute(sql);
...
async function addNewVacation(vacation: VacationModel): Promise<VacationModel>{
  ...
  const sql = `INSERT INTO vacationsproject.vacation VALUES(DEFAULT,
...
async function deleteVacation(id: number): Promise<void> {
  const sql = `DELETE FROM vacationsproject.vacation WHERE id = ${id}`;
  await dal.execute(sql);
}

async function updateFullVacation(vacation: VacationModel): Promise<VacationModel> {

  const sql = `UPDATE vacationsproject.vacation SET
...
async function getAllFollowedVacations(userId:number):Promise<SavedModel>{
  ...
  SELECT vacation.id, destination,
                    ...
                    FROM vacationsproject.vacation
...
async function addFollow(vacationToFollow: SavedModel): Promise<SavedModel> {
  ...
  const sqlVacationsTable = `UPDATE vacationsproject.vacation
